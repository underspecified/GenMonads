...
from genmonads.par_list import default_pool_settings
...
class PoolEvaluator(Generic[A]):
    def __init__(self,
        ...
        if pool_settings:
            ...
            self.pool_settings = default_pool_settings

    def __call__(self):
        chunksize = self.pool_settings.pop('chunksize', 1)
        with Pool(**self.pool_settings) as pool:
            pool.map = partial(pool.map, chunksize=chunksize)
            result = self.f(pool)
        return result


def pool_eval(thunk: Thunk[A],
              **pool_settings
              ) -> PoolEvaluator[A]:
    return PoolEvaluator(lambda pool: pool.apply(thunk), **pool_settings)


class ThunkEvaluator(Generic[A]):
    def __init__(self, f: Thunk[A]):
        self.f = f
...
def thunk_eval(thunk: Thunk[A]):
    return ThunkEvaluator(thunk)


evaluator = thunk_eval

Evaluator = Union[PoolEvaluator[A], ThunkEvaluator[A]]
...
class Eval(Monad,
    ...
    def map(self, f: F1[B, 'Eval[C]']) -> 'Eval[C]':
        """
        Applies a function to the inner value of a `Eval`.

        Args:
            f (F1[B, Eval[C]]): the function to apply
...
def now(x: Union[A, Evaluator[A]]) -> 'Now[A]':
    """
...
class Later(Eval[A]):
    ...
    def __init__(self, thunk: Evaluator[A]):
        self._thunk: Evaluator[A] = evaluator(thunk)
...
def later(thunk: Evaluator[A]) -> Later[A]:
    """
    ...
    Args:
        thunk (Evaluator[A]): the computation
...
def always(thunk: Evaluator[A]) -> Always[A]:
    """
...
class Call(Eval[A]):
    ...
    def _loop(fa: 'Call[A]') -> 'Call[A]':
        ...
        def go(_fa: Union['Call[A]', 'Compute[A]', Evaluator[A]]
               ) -> Union['Call[A]', 'Compute[A]', Evaluator[A]]:
            if _fa.is_call():
...
def defer(thunk: Evaluator[A]) -> 'Eval[A]':
    """
...
class Compute(Eval[A]):
    ...
    def get(self) -> A:
        def go(curr: Union['Compute[A]', Evaluator[A]],
               fs: typing.List[F1[A, Call[A]]]
               ) -> Union[A,
                          Tuple[Union['Compute[A]', typing.List[Evaluator[A]],
                                      typing.List[F1[A, Call[A]]]]]]:
            if curr.is_compute():
                cc = curr.start()
                if cc.is_compute():
                    return evaluator(lambda: go(cc.start(), [cc.run, curr.run] + fs))
                else:
                    return evaluator(lambda: go(curr.run(cc.get()), fs))
            elif fs:
                f, *rest = fs
                return evaluator(lambda: go(f(curr.get()), rest))
            else:
                return curr.get()
